 ...
 */
public class HashLeafPage extends HFPage{
   ...
   */
  public HashLeafPage(PageId pageno, int keyType) 
    throws ConstructPageException, ReplacerException, HashOperationException, PageUnpinnedException, InvalidFrameNumberException, PageNotReadException, BufferPoolExceededException, PagePinnedException, BufMgrException, IOException
    {
   ...
   *@exception ConstructPageException BTLeafPage constructor error
   */
  public HashLeafPage(Page page, int keyType) 
    throws IOException, 
   ...
   *@exception ConstructPageException BTLeafPage constructor error
   */
  public HashLeafPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super();
      try{
        ...
        e.printStackTrace();
	throw new ConstructPageException(e, "construct hash leaf page failed");
      }
      setType(NodeType.LEAF);
    }  
  ...
  public RID insertRecord(KeyClass key, RID dataRid) 
    throws  LeafInsertRecException
    {
      ...
      try {
        ...
        byte[] record=BT.getBytesFromEntry(entry);  
        RID rid=super.insertRecord(record);

        // //We are already checking in _insert
        //   if (rid==null)
        //   {
        // //    This means the page doesn't have enough space, now we'll create the overflow page
        //     PageId overflow_pageid = this.getNextPage();
        //     if(overflow_pageid.pid == INVALID_PAGE)
        //     {
        //       HashLeafPage overflow_page = new HashLeafPage(keyType);
        //       this.setNextPage(overflow_page.getCurPage());
        //       RID ret_rid = overflow_page.insertRecord(key, rid);
        //       return ret_rid;
        //     }
   ...
   */
  boolean redistribute(HashLeafPage leafPage, BTIndexPage parentIndexPage, 
