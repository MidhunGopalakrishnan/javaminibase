...
public class HashFile extends IndexFile implements GlobalConst {
	 ...
	 */
	public HashFile(String filename) throws GetFileEntryException, PinPageException, ConstructPageException {
	 ...
	 */
	public HashFile(String filename, int keytype, int keysize, int delete_fashion, float utilization_input)
...

	{
		...
		if (max_bucket_key_value < cur_bucket_key_value + 1) {
			...
			// We need to rehash!
			HashLeafPage new_bucket_page = new HashLeafPage(headerPage.get_keyType());
	...
	public ArrayList search(KeyClass key)
		...
		ArrayList<RID> ridInDataFile = new ArrayList<RID>();
		int hash_value = hash_record(key);
		int bucket_to_search = hash_value % (int) Math.pow(2, h_0 + level);
		if (bucket_to_search <= split_ptr_loc)
			bucket_to_search = hash_value % (int) Math.pow(2, h_0 + level + 1);
		...
		while (true) {
			for (temp_entry = bucket_page_1.getFirst(temp_rid); temp_entry != null; temp_entry = bucket_page_1
				...
				if (BT.keyCompare(key, temp_entry.key) == 0) // Check whether the keys are equal
				{
					...
					RID tupleRidInDataFile = ((LeafData) (temp_entry.data)).getData();
					ridInDataFile.add(new RID(new PageId(tupleRidInDataFile.pageNo.pid), tupleRidInDataFile.slotNo));
				}
			}
				...
				if (overflow_pageid.pid == INVALID_PAGE)
				{
					if(ridInDataFile.size()==0)
					return null;

					return ridInDataFile;
				}
	...
	void print_bucket(PageId bucket_id_1) {
		...
		try {
			...
			for (temp_entry = bucket_page_1.getFirst(temp_rid); temp_entry != null; temp_entry = bucket_page_1
				...
				if (headerPage.get_keyType() == AttrType.attrString)
					System.out.println(i + " (key, pageId, hashvalue):   (" + (StringKey) temp_entry.key + ",  "
