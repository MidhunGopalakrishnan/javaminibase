...
public class SortPref extends Iterator implements GlobalConst{
    ...
    private pnodeSplayPQ Q1;
    private pnodeSplayPQ Q2;
	...
	*/
    ...
    private int generate_runs(int max_elems, AttrType sortFldType, int count)
            ...
            JoinsException,
            Exception {
        System.out.println("GENERATE RUNS!!!");
        _count = count;
        Tuple tuple;
        pnode cur_node;
        //????????????????????????????????????????????????????????????
        pnodeSplayPQ Q1 = new pnodeSplayPQ(_pref_list[0], sortFldType, order);
        pnodeSplayPQ Q2 = new pnodeSplayPQ(_pref_list[0], sortFldType, order);
        //pnodeSplayPQ Q3 = new pnodeSplayPQ(_pref_list[2], sortFldType, order);
        //pnodeSplayPQ Q4 = new pnodeSplayPQ(_pref_list[2], sortFldType, order);
      ...
      /* ??????????????????????????????????????????
       ...
       CompareTupleWithTuplePref(Tuple t1, AttrType[] type1, Tuple t2,
                                                 AttrType[] type2,
                                                 short len_in,
                                                 short[] str_sizes,
                                                 int[] pref_list,
       ...
       */
            /*
            int[] temp_pref_list = new int[2];
            for (int i = 0; i < _pref_list_len; i++){
                temp_pref_list[i] = _pref_list[i] - 1;
            }
             */
            ...
            if ((comp_res < 0 && order.tupleOrder == TupleOrder.Ascending) || (comp_res > 0 && order.tupleOrder == TupleOrder.Descending)) {
                ...
                try {
                    ...
                    //System.out.println("LASTELEM: " + lastElem.getIntFld(_pref_list[0]));
                    //lastElem.print(_in);
                    //System.out.println("Cur_node.tuple(enq() to pother_Q): ");
                    //cur_node.tuple.print(_in);
                } catch (UnknowAttrType e) {
                    throw new SortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
                }
                ...
                //System.out.println("ELSE: comp_res, tupleOrder!!");
                // set lastElem to have the value of the current tuple,
                // need tuple_utils.java
                //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
                TupleUtils.SetValue(lastElem, cur_node.tuple, _pref_list[0]+1, sortFldType);
            ...
            if (p_elems_other_Q == max_elems) {
                System.out.println("check whether the other queue is full");
            ...
            // now check whether the current queue is empty
                ...
                System.out.println("check whether the current queue is empty");
                while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
                    try {
                        tuple = _am.get_next();  // according to Iterator.java
                    } catch (Exception e) {
        ...
        System.out.println("\nn_tuples[run_num] " + n_tuples[run_num] + "run_num " + run_num);
        count++;
        run_num++;
	...
	*/
    ...
    private void MIN_VAL(Tuple lastElem, AttrType sortFldType)
        ...
        switch (sortFldType.attrType) {
            case AttrType.attrInteger:
                ...
                lastElem.setIntFld(_pref_list[0]+1, Integer.MIN_VALUE);
                //lastElem.setIntFld(_sort_fld, Integer.MIN_VALUE);
                break;
            case AttrType.attrReal:
                //      lastElem.setHdr(fld-no, junk, null);
                lastElem.setFloFld(_pref_list[0]+1, Float.MIN_VALUE);
    ...
    private void MAX_VAL(Tuple lastElem, AttrType sortFldType)
        ...
        switch (sortFldType.attrType) {
            case AttrType.attrInteger:
                //      lastElem.setHdr(fld_no, junk, null);
                lastElem.setIntFld(_pref_list[0], Integer.MAX_VALUE);
                break;
            case AttrType.attrReal:
                //      lastElem.setHdr(fld_no, junk, null);
                lastElem.setFloFld(_pref_list[0]+1, Float.MAX_VALUE);
                break;
            case AttrType.attrString:
                //      lastElem.setHdr(fld_no, junk, s_size);
                lastElem.setStrFld(_pref_list[0], s);
    ...
    ) throws IOException, SortException
    {
        ...
        //{
            ...
            //if(i == 1)
            // Q1 = new pnodeSplayPQ(pref_list[i], in[i], order);
            //if(i == 2)
            //Q2 = new pnodeSplayPQ(pref_list[i], in[pref_list[i] -1], order);
        //}
    ...
    public Tuple get_next()
        ...
        if (first_time) {
            ...
            //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
            /*
            Nruns = new int[_pref_list_len];
            for(int i = 0; i < _pref_list_len; i++)
            {
                Nruns[i] = generate_runs(max_elems_in_heap, _in[i], i);
                System.out.println("Generated " + Nruns[i] + " runs");
            }
             */

            //Nruns = generate_runs(max_elems_in_heap, _in[_pref_list[0]], sortFldLen);
            ...
            /*
            for(int i = 0; i < _pref_list_len;i++) {
                setup_for_merge(tuple_size, Nruns[i]);
            }
             */
///*
            System.out.println(Nruns);
